...
const DebaterCard: React.FC<DebaterCardProps> = ({ debater, isActive, side, displayName, displayRole, speakingLabel }) => {
  ...
  return (
    ...
    >
      <img
        src={debater.avatar}
        alt={displayName ?? debater.name}
        className="w-14 h-14 sm:w-20 sm:h-20 rounded-full border-2 border-slate-600 object-cover shadow-lg"
      />
      <div className="text-center">
        <p className="font-bold text-[11px] sm:text-sm truncate w-24 sm:w-32">{displayName ?? debater.name}</p>
